...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         if (weight < 0) throw new IllegalArgumentException("Weight cannot be less than zero");
         ...
         for(char letter: word.toCharArray()){
            ...
            Node child = currentNode.getChild(letter);
                
            if(child == null){
               child = new Node(letter, currentNode, weight);
               currentNode.children.put(letter, child);
            }
            currentNode = child;
         }
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         for(char letter: prefix.toCharArray()) {
            ...
            if(currentNode.getChild(letter) == null){
               return new ArrayList<String>();
            }
         ...
         if (currentNode.isWord) {
            matches.add(new Term(currentNode.myWord, currentNode.myWeight));
         }
         ...
         matches.sort(new Term.ReverseWeightOrder());
         ArrayList<Term> kMatches = new ArrayList<Term>();
         if(k > matches.size()) {
            kMatches.addAll(matches);
         }
         else {
            kMatches.addAll(matches.subList(0, k));
         }
         
         ArrayList<String> topKMatches = new ArrayList<String>();
         for(Term t: kMatches) {
            topKMatches.add(t.getWord());
         }
      ...
      public String topMatch(String prefix) {
         ...
         while(!currentNode.isWord && currentNode.getWeight() != currentNode.mySubtreeMaxWeight) {  
            for(Node child: currentNode.children.values()) {
               if (child.mySubtreeMaxWeight == currentNode.mySubtreeMaxWeight) {
                  currentNode = child;
                  break;
               }
            }
